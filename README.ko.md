...
### 왜 플러그인이 오케스트레이터 모델을 대신 바꿀 수 없나

Claude Code 플러그인은 `model` frontmatter로 **서브에이전트의** 모델은 고정할 수
있지만, **메인 세션의** 모델은 사용자가 `/model` 커맨드로만 바꿀 수 있습니다. 그래서
이 플러그인은:

- **`/orchestrator-model`** → 선택을 프로젝트 기본값으로 저장 (다음 세션부터 자동 적용).
- **SessionStart 훅** → 저장된 기본값과, 지금 바로 적용하는 방법을 안내.
- 지금 세션에 즉시 적용하려면 직접 `/model opus` 또는 `/model fable` 입력.
...
## 선택: Codex를 동료 리뷰어로 추가

다른 모델 계열의 교차 검증을 위해 OpenAI 공식 Codex 플러그인을 설치하세요:
...
Node.js 18.18+ 와 ChatGPT 계정(Free 포함) 또는 OpenAI API 키가 필요합니다. 설치 후
`orchestration-protocol`이 중요한 설계 결정/diff에 대해 `/codex:review` 또는
`/codex:adversarial-review`를 *제안*합니다. Codex 명령은 별도 사용량을 소모하므로,
오케스트레이터가 임의로 실행하지 않고 확인을 받습니다.
